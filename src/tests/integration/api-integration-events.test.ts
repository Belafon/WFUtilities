...
suite('EventManager Integration Tests', () => {
  ...
  suite('POST /api/event/:eventId/open - Open Event', () => {
    ...
    test('should handle editor failures gracefully', async () => {
      ...
      // Make openFile throw an error for this test
      openFileStub.rejects(new Error('Editor crashed'));
      ...
      assert.strictEqual(response.body.success, false);
      assert.ok(response.body.error.includes('Failed to open event'));
  ...
  suite('POST /api/event/:eventId/setTime - Set Event Time', () => {
    ...
    test('should return 400 for invalid time range data', async () => {
      ...
      assert.strictEqual(response.body.success, false);
      assert.ok(response.body.error.includes('Invalid timeRange'));
    });
  ...
  suite('Test Environment Mode', () => {
    ...
    test('should return demo responses in test mode', async () => {
